...
feature 'Delete own attachment', %q{
  ...
  given!(:authors_answer) { create :answer, :with_attachment, question: authors_question, user: author }
  given!(:visitors_answer) { create :answer, :with_attachment, question: authors_question, user: visitor }
  ...
  describe 'Author', js: true do
    ...
    scenario 'deletes own attachment' do
      ...
      within('#files-attached') do
        find("#file_id-#{authors_answer.files.first.id}").click_on 'Remove'
        page.driver.browser.switch_to.alert.accept
      end
      expect(page).not_to have_content authors_answer.files.first.filename.to_s
    ...
    scenario 'tries to delete visitors attachment' do
      within('#answers-table') do
        ...
        find("#answer_id-#{visitors_answer.id}").click_on 'Files'
        expect(find("#file_id-#{visitors_answer.files.first.id}")).not_to have_link 'Remove'
      end
  ...
  describe 'Visitor', js: true do
    scenario 'Unauthenticated user tries to delete an attachment' do
      ...
      within('#answers-table') do
        ...
        find("#answer_id-#{authors_answer.id}").click_on 'Files'
      end
